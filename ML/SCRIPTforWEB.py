def ml_model(inputer):
    ...
    # %%
    ordinal = pd.read_csv("ML/base_ordinal_LIMPA.csv", index_col = 0)
    ...
    logreg_pred_threshold = np.where(logreg.predict_proba(train_data)[:,1] > THRESHOLD, 1, 0)
    logreg_pred_proba = logreg.predict_proba(train_data)

    xgb = XGBClassifier().fit(train_data, train_labels)
    xgb_pred_threshold = np.where(xgb.predict_proba(train_data)[:,1] > THRESHOLD, 1, 0)
    xgb_pred_proba = xgb.predict_proba(train_data)

    kneighbors = KNeighborsClassifier(n_neighbors = 5).fit(train_data, train_labels)
    kneighbors_pred_threshold = np.where(kneighbors.predict_proba(train_data)[:,1] > THRESHOLD, 1, 0)
    kneighbors_pred_proba = kneighbors.predict_proba(train_data)
    ...
    inputFromWeb['prob_default_60d'] = pd.DataFrame(average_probability_prediction_default_60d)[0]
    ...
    inputFromWeb['estimativa_juros'] = estimativa_juros
